        ...
        case(CS)
            ...
            READ: begin
                if (miss && !write_back) begin  // no need to write back before evicting
                    ...
                    NS = READ;
                end else if (w_req) begin  // hit, and receive a w_req
                    NS = WRITE;
                end else begin
                    NS = IDLE;
                end
    ...
    always @(*) begin
        ...
        case(CS)
            IDLE: begin
                ...
                miss = 1'b0;
                ret_buf_we = 1'b0;
                if (refill) begin  // write missed line (read from memory) into cache
            ...
            READ: begin
                data_from_mem = 1'b0;  // read data from cache
                if (hit) begin // 命中
                    miss = 1'b0;
            ...
            MISS: begin
                miss = 1'b1;
                mem_r = 1'b1;  // read missed line from memory
                mem_addr = addr_buf;
                if (mem_ready) begin  // wait until the memory finish reading
                    mem_r = 1'b0;
                    ret_buf_we = 1'b1;  // ok to return read data
                end 
            end
            WRITE: begin
                data_from_mem = 1'b0;  // read data from cache
            ...
            W_DIRTY: begin
                miss = 1'b1;
                mem_w = 1'b1;  // write dirty line back to memory
                mem_addr = dirty_mem_addr_buf;
                mem_w_data = dirty_mem_data_buf;
                if (mem_ready) begin  // wait until the memory finish writing
                    mem_w = 1'b0;
                end
            end
            default:;
